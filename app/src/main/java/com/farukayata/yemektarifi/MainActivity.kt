        ...
        setContent {
            YemekTarifiTheme {
                ...
                NavHost(navController = navController, startDestination = "login") {
                    ...
                    composable(
                        route = "homeRecipeDetail/{recipeName}",
                        arguments = listOf(navArgument("recipeName") { type = NavType.StringType })
                    ) { backStackEntry ->
                        val encodedName = backStackEntry.arguments?.getString("recipeName") ?: ""
                        val recipeName = java.net.URLDecoder.decode(encodedName, java.nio.charset.StandardCharsets.UTF_8.toString())
                        val popularRecipes = viewModel.popularRecipes.collectAsState().value
                        val recipe = popularRecipes.find { it.name == recipeName }
